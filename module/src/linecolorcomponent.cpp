...
namespace nap
{
	...
	{
		mResource = getComponent<LineColorComponent>();

		// Set intensity and color values
		mColorOne = mResource->mColorOne.get();
		mColorTwo = mResource->mColorTwo.get();
		...
		// Copy over intensity
		mIntensitySmoother.setValue(mResource->mIntensity->mValue);
	...
	void LineColorComponentInstance::update(double deltaTime)
	{
		...
		mIntensitySmoother.update(mResource->mIntensity->mValue, deltaTime);
